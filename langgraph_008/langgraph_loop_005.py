...
g.add_edge("b", "c")
g.add_edge("b", "d")
g.add_edge(["c", "b"], "a")  # 🔥 修复：不是 add_node，是 add_edge！
# 编译
gc = g.compile()

# 运行
respa = gc.invoke({"nodes": []})
print("\n✅ 最终结果：", respa)
# ======================
# ✅ 正确绘图（用编译后的 gc）
# ======================
try:
    png_data = gc.get_graph().draw_mermaid_png()
    with open("langgraph_loop.png", "wb") as f:
        f.write(png_data)
except:
    print("绘图需要安装：pip install pillow playwright")
